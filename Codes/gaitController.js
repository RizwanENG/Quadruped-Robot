...
class GaitController {
    ...
    calibrate() {
        ...
        Object.keys(motors).forEach(key => {
            this.servoController.setAngle(
                motors[key].channel,
                motors[key].defaultAngle
            )
        })
    }
    ...
    setLegPosition(legId, angles) {
        ...
        this.servoController.setAngle(
            motors[`${legId}_SHOULDER`].channel,
            angles.thetaShoulder
        );

        this.servoController.setAngle(
            motors[`${legId}_ELBOW`].channel,
            angles.thetaElbow
        );

        this.servoController.setAngle(
            motors[`${legId}_HIP`].channel,
            angles.thetaHip
        );
    ...
    move(controller) {
        ...
        const loop = () => {
            ...
            const curvePoints =
                applyMomentumToCurve3d(momentum, basicGait)

            const {fr, fl, br, bl} =
                mapCoordsToLegs(Math.floor(index), curvePoints)
